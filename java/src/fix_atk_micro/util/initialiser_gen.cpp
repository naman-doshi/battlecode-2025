...
const int RAD_SQ = 200;
const int MAX_BLOCK_IND = SIZE / BLOCK_SIZE;

const str SSIZE = to_string(SIZE);
const str SBLOCK_SIZE = to_string(BLOCK_SIZE);
const str SRAD_SQ = to_string(RAD_SQ);
const str SMAX_BLOCK_IND = to_string(MAX_BLOCK_IND);
const str SMAX_BLOCK_INDM = to_string(MAX_BLOCK_IND - 1);
...
str cap(str name) {
    str res = name;
    res[0] = toupper(name[0]);
    return res;
}

str bool_name(str name) {
    return name + "Bool";
}

str init_name(str name) {
    return "hasInit" + cap(name);
}

str get(str name, int i) {
    return name + '[' + to_string(i) + ']';
}

str get(str name, char c) {
    return name + '[' + c + ']';
}

str get(str name, str s) {
    return name + '[' + s + ']';
}
...
void print_fields() {
    str pfx = "\t";
    each (name, val, stuff) {
        println(pfx + "private static boolean " + init_name(name) + " = false;");
        println(pfx + "private static boolean[][] " + bool_name(name) + ";");
        println(pfx + "private static int[][] " + name + ";");
    }
}
...
void print_makes() {
    str pfx = "\t";
    each (name, val, stuff) {
        println(pfx + "public static int[][] get" + cap(name) + "() {");
        println(pfx + '\t' + init_name(name) + " = true;");
        println(pfx + '\t' + name + " = new " + get(get("int", SSIZE), SSIZE) + ";");
        println(pfx + '\t' + bool_name(name) + " = new " + get(get("boolean", SMAX_BLOCK_IND), SMAX_BLOCK_IND) + ";");

        println("\t\tint posX = Game.rc.getLocation().x - " + to_string(BLOCK_SIZE / 2) + ";\n\t\tint posY = Game.rc.getLocation().y - " + to_string(BLOCK_SIZE / 2) + ";\n\t\tfor (int x = " + SMAX_BLOCK_INDM + "; x >= 0; x--) {\n\t\t\tfor (int y = " + SMAX_BLOCK_INDM + "; y >= 0; y--) {\n\t\t\t\tif ((" + SBLOCK_SIZE + " * x - posX) * (" + SBLOCK_SIZE + " * x - posX) + (" + SBLOCK_SIZE + " * y - posY) * (" + SBLOCK_SIZE + " * y - posY) <= " + SRAD_SQ + ") {");
...
void print_upd() {
    println("\tpublic static void upd() {\n\t\tint posX = Game.rc.getLocation().x - " + to_string(BLOCK_SIZE / 2) + ";\n\t\tint posY = Game.rc.getLocation().y - " + to_string(BLOCK_SIZE / 2) + ";\n\t\tfor (int x = " + SMAX_BLOCK_INDM + "; x >= 0; x--) {\n\t\t\tfor (int y = " + SMAX_BLOCK_INDM + "; y >= 0; y--) {\n\t\t\t\tif ((" + SBLOCK_SIZE + " * x - posX) * (" + SBLOCK_SIZE + " * x - posX) + (" + SBLOCK_SIZE + " * y - posY) * (" + SBLOCK_SIZE + " * y - posY) <= " + SRAD_SQ + ") {");
...
main() {
    ...
    println("package fix_atk_micro.util;\n\nimport fix_atk_micro.Game;\n\npublic class Initialiser {");
    print_fields();
    print_makes();
    print_upd();
    println("}");
